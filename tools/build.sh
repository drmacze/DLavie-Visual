...
# Critical loader fix: vanilla client biomes must use minecraft:<biome> identifiers
# and a schema version supported by the current vanilla resource pack.
python3 -B tools/normalize_vv_bindings.py
...
# Feature-level validators + cross-system runtime audit.
python3 -B tools/validate_pack.py
python3 -B tools/validate_vv_bindings.py
